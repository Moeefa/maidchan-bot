module.exports = {
  "PERMISSIONS_ADMINISTRATOR": "Administrator",
  "PERMISSIONS_CREATE_INSTANT_INVITE": "Create a instant invite",
  "PERMISSIONS_KICK_MEMBERS": "Kick members",
  "PERMISSIONS_BAN_MEMBERS": "Ban members",
  "PERMISSIONS_MANAGE_CHANNELS": "Manage channel",
  "PERMISSIONS_MANAGE_GUILD": "Manage guild",
  "PERMISSIONS_ADD_REACTIONS": "Add reactions",
  "PERMISSIONS_VIEW_AUDIT_LOGS": "View Audit Logs",
  "PERMISSIONS_PRIORITY_SPEAKER": "Priority speaker",
  "PERMISSIONS_VIEW_CHANNEL": "View channel",
  "PERMISSIONS_SEND_MESSAGES": "Send messages",
  "PERMISSIONS_SEND_TTS_MESSAGES": "Send tts (text to speech) messages",
  "PERMISSIONS_MANAGE_MESSAGES": "Manage messages",
  "PERMISSIONS_EMBED_LINKS": "Embed links",
  "PERMISSIONS_ATTACH_FILES": "Attach files",
  "PERMISSIONS_READ_MESSAGE_HISTORY": "View message history",
  "PERMISSIONS_MENTION_EVERYONE": "Mention everyone",
  "PERMISSIONS_USE_EXTERNAL_EMOJIS": "Use external emojis",
  "PERMISSIONS_CONNECT": "Connect in a voice channel",
  "PERMISSIONS_SPEAK": "Speak in a voice channel",
  "PERMISSIONS_MUTE_MEMBERS": "Mute members",
  "PERMISSIONS_DEAFEN_MEMBERS": "Deafen members",
  "PERMISSIONS_MOVE_MEMBERS": "Move members",
  "PERMISSIONS_USE_VAD": "Voice activity detection",
  "PERMISSIONS_CHANGE_NICKNAME": "Change nickname",
  "PERMISSIONS_MANAGE_NICKNAMES": "Manage nicknames",
  "PERMISSIONS_MANAGE_ROLES": "Manage roles",
  "PERMISSIONS_MANAGE_WEBHOOKS": "Manage webhooks",
  "PERMISSIONS_MANAGE_EMOJIS": "Manage emojis",
  "PERMISSIONS_MEMBER_NO_PERM": "{0}, You don't have permission (``{1}``) to use this command!",
  "PERMISSIONS_CLIENT_NO_PERM": "{0}, Sorry, I don't have permission (``{1}``) to execute this command! 🙇🏻‍",
  "PING_RESPONSES": [`I did it? I-I arrived on time?`, `Seriously? I wasted {0}ms with you...`],
  "PING_BOT_LATENCY": "Bot's latency",
  "PING_API_LATENCY": "Discord API's latency"
}